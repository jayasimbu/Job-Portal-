    ...
    <ResumeProvider>
      <div className="h-screen flex overflow-hidden bg-slate-50 dark:bg-[#0a0f14] transition-colors font-sans">
        ...
        <div className={`flex-1 flex flex-col min-w-0 ${!isHome ? 'md:pl-[270px]' : ''}`}>
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  <div className="text-right hidden sm:block">
                    <p className="text-[15px] font-bold text-slate-900 dark:text-white leading-tight group-hover:text-blue-600 transition-colors">{user?.full_name || 'User'}</p>
                    <div className="flex items-center justify-end gap-1.5 mt-0.5">
                       <span className="text-[9px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest leading-tight">
                         {user?.is_verified ? 'Verified Expert' : 'Standard Profile'}
                       </span>
                       <div className={`size-3.5 ${user?.is_verified ? 'bg-blue-600' : 'bg-slate-300'} rounded-full flex items-center justify-center`}>
          ...
          <main id="main-content" className="flex-1 overflow-y-auto px-4 md:px-6 lg:px-6 pb-4 scroll-smooth">
