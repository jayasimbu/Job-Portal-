...
export default function StatCard({ title, value, icon, change, trend = "up", className = "" }) {
  ...
  return (
    <Card className={`${UI.CARD_BASE} ${UI.CARD_FLEX} ${UI.STAT_CARD} relative overflow-hidden bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-[#0f172a] hover:border-blue-300 dark:hover:border-blue-600 hover:-translate-y-1 hover:shadow-2xl hover:shadow-blue-500/10 transition-all group ${className}`}>
      {/* Background Subtle Glow */}
      <div className="absolute -top-10 -right-10 w-32 h-32 bg-blue-500/5 dark:bg-blue-600/10 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
