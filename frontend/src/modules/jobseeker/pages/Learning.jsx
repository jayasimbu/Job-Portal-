    ...
    <div className="space-y-6 pb-16 pt-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <div className="lg:col-span-8 space-y-5">
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {courses.map((course, idx) => (
                <div key={idx} className="bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 group flex flex-col h-full">
                  <div className="flex-1">
                     ...
                     <div className="flex flex-wrap gap-1.5 text-[10px] font-bold">
                        <span className="bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-slate-600 dark:text-slate-400">{course.duration || 'Flexible'}</span>
                  ...
                  <a href={course.url || "#"} target="_blank" rel="noreferrer" className="mt-4 w-full flex items-center justify-center gap-2 py-2 text-[10px] font-bold text-slate-500 hover:text-blue-600 uppercase tracking-widest rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
          ...
          {/* SKILL ROADMAP */}
          <section className="space-y-3 pt-4 border-t border-slate-200 dark:border-slate-700/50">
            <div className="flex items-center gap-2">
            ...
            <div className="space-y-3">
              {roadmap.map((item, idx) => (
                <div key={idx} className="bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
           ...
           <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 shadow-sm">
             ...
             <div className="space-y-3">
               {certifications.map((cert, idx) => (
                 <div key={idx} className="bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl p-3 flex flex-col gap-1.5 hover:border-amber-300/50 transition-colors">
