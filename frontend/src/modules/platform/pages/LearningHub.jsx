    ...
    <div className="h-full flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="flex-shrink-0 mb-6">
        <div className="flex items-center gap-3 mb-1">
           ...
           <h2 className="text-[10px] font-black text-blue-600 dark:text-blue-400 tracking-[0.2em] uppercase">AI Growth Engine</h2>
        </div>
        <h1 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight uppercase">AI Learning Hub</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400">Personalized curriculum to close your detected skill gaps.</p>
      </header>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 pb-6">
            {recommendations.map(course => (
              <div key={course.id} className="group p-6 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl hover:border-blue-500 transition-all shadow-sm flex flex-col relative overflow-hidden">
                <div className="flex justify-between items-start mb-6 relative z-10">
                  <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${course.status === 'In Progress' ? 'bg-blue-50 text-blue-600' : 'bg-green-50 text-green-600'}`}>
                    {course.status}
                  </div>
                ...
                <div className="mb-4 relative z-10">
                  <h3 className="text-lg font-black text-slate-900 dark:text-white group-hover:text-blue-600 transition-colors uppercase tracking-tight leading-tight">{course.title}</h3>
                  <p className="text-xs font-bold text-slate-500 uppercase mt-1">{course.provider} • {course.duration}</p>
                </div>

                <div className="p-5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800 mb-6 relative z-10">
                  ...
                  <div className="mb-6 flex-1 relative z-10">
                    ...
                    <div className="space-y-2">
                      {course.videos.map((vid, idx) => (
                        <div key={idx} className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group/vid cursor-pointer" onClick={() => setSelectedVideo(vid)}>
                          <div className="w-20 h-12 bg-slate-200 dark:bg-slate-800 rounded overflow-hidden shrink-0 relative">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-[11px] font-bold text-slate-700 dark:text-slate-300 truncate group-hover/vid:text-blue-600 transition-colors">{vid.title}</p>
                ...
                {course.progress > 0 && (
                  <div className="mb-6 relative z-10">
                    ...
                    <div className="h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden shadow-inner">
                      <div className="h-full bg-blue-600 rounded-full transition-all duration-1000 shadow-[0_0_10px_rgba(37,99,235,0.4)]" style={{ width: `${course.progress}%` }} />
                    </div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-3 relative z-10 mt-auto">
                  <button 
                    ...
                    className="h-11 bg-red-50 hover:bg-red-100 dark:bg-red-500/10 dark:hover:bg-red-500/20 text-red-600 font-black text-[10px] uppercase tracking-widest rounded-xl transition-all flex items-center justify-center gap-2 active:scale-95 border border-red-200/50 dark:border-red-500/20"
                  >
                  ...
                  <button 
                    ...
                    className="h-11 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-blue-600 dark:hover:bg-blue-600 hover:text-white transition-all flex items-center justify-center gap-2 shadow-sm active:scale-95"
                  >
          ...
          <EmptyState 
            ...
            onAction={() => navigate('/platform/dashboard')}
          />
        )}
      </div>
